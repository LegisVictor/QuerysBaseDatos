...
where  uc.FechaCreacion between '2023-05-01 00:00' and '2023-09-01 00:00' and Cliente='GestionHumana' --- hasta 31-03-2023
...
where  uc.FechaCreacion >'2020-01-01 00:00' and Cliente='GestionHumana' and s.Sil_CodSuscripcion is not null  and s.FechaFin>getdate() and ac.CodAplicacionWeb in (10)
)
...
from 
  (
    select 
      REPLACE(
        USER_ID, 'f:bd03591f-be73-4be4-b7ed-a7c264d01cd9:', 
        ''
      ) as Usuario, 
      FechaCreacion, 
  ...
  Left Join [10.250.4.29].[keycloak].[dbo].BROKER_LINK BL on REPLACE(
    BL.USER_ID, 'f:bd03591f-be73-4be4-b7ed-a7c264d01cd9:', 
    ''
  ) = usrs.Usuario 
  Left join (
